...
describe("Seeder — scripts/seed-loadtest.ts (Task 2)", () => {
  const seederPath = resolve(APP_ROOT, "scripts/seed-loadtest.ts");
  let seederContent = "";
...
describe("k6 HTTP load test scripts — existence (Task 3)", () => {
  ...
  for (const script of scripts) {
    it(`${script} exists`, () => {
      expect(existsSync(resolve(APP_ROOT, script))).toBe(true);
    });
  }
});

describe("k6 scripts — threshold config exported (Task 3.7)", () => {
  const apiEndpointsPath = resolve(APP_ROOT, "tests/load/scenarios/api-endpoints.js");
  let content = "";
...
describe("k6 event-spike.js — dedicated spike profile (Task 3.5, 3.6)", () => {
  const path = resolve(APP_ROOT, "tests/load/scenarios/event-spike.js");
  let content = "";
...
describe("WebSocket load test — ws-loadtest.mjs (Task 4)", () => {
  const wsPath = resolve(APP_ROOT, "tests/load/scenarios/ws-loadtest.mjs");
  let wsContent = "";
...
describe("Thresholds config — tests/load/config/thresholds.js (Task 5)", () => {
  const thresholdsPath = resolve(APP_ROOT, "tests/load/config/thresholds.js");
  let content = "";
...
describe("Report generator — tests/load/lib/report.mjs (Task 5)", () => {
  const reportPath = resolve(APP_ROOT, "tests/load/lib/report.mjs");
  let content = "";
...
describe("package.json — load test scripts (Task 5.4)", () => {
  ...
  it("test:load:seed uses npx tsx (not bun or npm)", () => {
    expect(pkg.scripts?.["test:load:seed"]).toContain("npx tsx");
  });
...
describe("Load test results directory (Task 2.7)", () => {
  it("tests/load/results/.gitkeep exists", () => {
    expect(existsSync(resolve(APP_ROOT, "tests/load/results/.gitkeep"))).toBe(true);
  });
