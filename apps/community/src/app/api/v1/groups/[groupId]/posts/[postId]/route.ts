// DELETE /api/v1/groups/[groupId]/posts/[postId] — leader/creator remove group post
import { withApiHandler } from "@/server/api/middleware";
import { successResponse } from "@/lib/api-response";
import { ApiError } from "@/lib/api-error";
import { requireAuthenticatedSession } from "@/services/permissions";
import { getGroupById, getGroupMember } from "@/db/queries/groups";
import { softDeleteGroupPost } from "@/db/queries/posts";
...
const deleteHandler = async (request: Request) => {
  ...
  if (!membership || (membership.role !== "creator" && membership.role !== "leader")) {
    throw new ApiError({
      title: "Forbidden",
      status: 403,
      detail: "Only group creators or leaders can remove group posts",
    });
  }

  const deleted = await softDeleteGroupPost(postId, groupId);
  if (!deleted) {
