...
export const PATCH = withApiHandler(async (req: Request): Promise<Response> => {
  ...
  // 404-not-403 to prevent information leakage (established pattern)
  ...
  // Delegate to state machine — enforces transition rules, writes audit trail, emits event
  await transition(idValidation.data, parsed.data.status as never, session.user.id, "employer");
