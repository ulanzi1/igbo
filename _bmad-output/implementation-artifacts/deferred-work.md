...
## Deferred from: code review of p-6-5-viewed-by-employer-signal-outbox-pattern round 2 (2026-05-02)

- W5: Events stuck in `'processing'` status if poller process crashes mid-batch — `claimPendingOutboxEvents` atomically transitions rows to `'processing'` but there is no recovery query that resets stale `'processing'` rows back to `'pending'`. Accumulate silently; invisible to the partial index `WHERE status = 'pending'`. Future: add a periodic query `WHERE status = 'processing' AND created_at < now() - interval '5 minutes'` to reset orphaned rows. (`portal-outbox.ts`)
- W6: Unknown future event types silently consumed as `'processed'` — if a new outbox event type is inserted before the poller code is deployed (rolling deploy window), those events are permanently lost; only a `console.warn` is emitted. Future: add an alerting hook or emit a metric when unknown event types are encountered. (`outbox-poller.ts`)
- W7: `cleanupProcessedOutboxEvents` accepts 0 or negative `olderThanDays` without validation — `olderThanDays = 0` evaluates to `processed_at < now()`, deleting all processed events immediately. Current route hardcodes `7`; risk is low but the function has no guard. Future: add `if (olderThanDays <= 0) throw new Error(...)` or `Math.max(1, olderThanDays)`. (`portal-outbox.ts`, `outbox/cleanup/route.ts`)
- W8: `getCompanyByOwnerId` uses `.limit(1)` without `ORDER BY` — non-deterministic if an employer ever has >1 company profile. Authorization check in `recordApplicationView` could fail incorrectly. Portal onboarding enforces 1:1 at application level but no DB unique constraint exists on `(owner_user_id)` in `portal_company_profiles`. Future: add DB unique constraint or add ORDER BY to make query deterministic. (`portal-companies.ts`, `application-view-service.ts`)

## Deferred from: code review of p-6-6-daily-weekly-digest (2026-05-02)
